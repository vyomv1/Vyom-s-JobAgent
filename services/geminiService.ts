...
export const analyzeJob = async (job: Job): Promise<JobAnalysis> => {
  ...
  const prompt = `
    ...
    **CRITICAL**: ADDRESS THE USER AS "YOU". DO NOT USE "VYOM".
    ...
    - **Verdict**: Write a 1-sentence summary explaining why it fits **your** profile. Use "Your".
...
export const expandJobDescription = async (job: Job): Promise<string> => {
    ...
    const prompt = `
      ...
      TASK: This job description is truncated. Based on the role title and company, generate a REALISTIC and DETAILED full job description. 
      Infer the likely Responsibilities, Requirements (Hard/Soft Skills), and "Nice to Haves".
      
      Output a clean, professional job spec in plain text (no markdown formatting symbols like ##, just nice paragraphs and bullet points using unicode bullets).
      Add a disclaimer at the bottom: "[Generated by AI based on role profile]"
    `;
    const response = await retryWithBackoff<GenerateContentResponse>(() => ai.models.generateContent({
        model: "gemini-2.5-flash",
        contents: prompt
    }));
    return response.text || job.summary;
};
