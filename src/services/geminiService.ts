...
export const generateApplicationKit = async (job: Job, analysis: JobAnalysis): Promise<string> => {
  const ai = getClient();
  const prompt = `
    Create a complete "Application Strategy Kit" for the user applying to:
    Role: ${job.title} at ${job.company}.
    Job Summary: ${job.summary}
    Your Strategy: ${analysis.strategy}
    
    Output structured Markdown, addressing the user as "You" (Not Vyom).
    Sections: # 🎯 Application Strategy, # 📄 Resume Adjustments, # 🎤 Interview Prep, # ✉️ Draft Cover Letter.
  `;
